...
public interface IStoragePostgreSQL {
  ...
  // RepairRun
  //
  String SQL_REPAIR_RUN_ALL_FIELDS_NO_ID =
  ...
  String SQL_REPAIR_RUN_ALL_FIELDS =
      "repair_run.id, " + SQL_REPAIR_RUN_ALL_FIELDS_NO_ID;
  String SQL_INSERT_REPAIR_RUN =
  ...
  String SQL_REPAIR_SEGMENT_ALL_FIELDS_NO_ID =
      ...
      + "end_time, fail_count";
  String SQL_REPAIR_SEGMENT_ALL_FIELDS =
  ...
  String SQL_GET_NEXT_FREE_REPAIR_SEGMENT_IN_WRAPPING_RANGE =
      ...
      + "(start_token >= :startToken AND end_token <= :endToken)) "
      + "ORDER BY fail_count ASC, start_token ASC LIMIT 1";
  String SQL_DELETE_REPAIR_SEGMENTS_FOR_RUN =
  ...
  String SQL_REPAIR_SCHEDULE_ALL_FIELDS =
  ...
  String SQL_GET_REPAIR_SCHEDULE =
      "SELECT " + SQL_REPAIR_SCHEDULE_ALL_FIELDS + " FROM repair_schedule WHERE id = :id";
  String SQL_GET_REPAIR_SCHEDULES_FOR_CLUSTER =
  ...
  String SQL_GET_REPAIR_SCHEDULES_FOR_KEYSPACE =
      "SELECT " + SQL_REPAIR_SCHEDULE_ALL_FIELDS + " FROM repair_schedule, repair_unit "
      + "WHERE repair_schedule.repair_unit_id = repair_unit.id AND keyspace_name = :keyspaceName";
  String SQL_GET_REPAIR_SCHEDULES_FOR_CLUSTER_AND_KEYSPACE =
      "SELECT " + SQL_REPAIR_SCHEDULE_ALL_FIELDS + " FROM repair_schedule, repair_unit "
      + "WHERE repair_schedule.repair_unit_id = repair_unit.id AND cluster_name = :clusterName "
      + "AND keyspace_name = :keyspaceName";

  String SQL_GET_ALL_REPAIR_SCHEDULES =
  ...
  // Utility methods
  //
  String SQL_GET_REPAIR_RUN_IDS_FOR_CLUSTER =
      "SELECT id FROM repair_run WHERE cluster_name = :clusterName";
  String SQL_SEGMENT_AMOUNT_FOR_REPAIR_RUN =
  ...
  Cluster getCluster(@Bind("name") String clusterName);
  ...
  int insertCluster(@BindBean Cluster newCluster);

  @SqlUpdate(SQL_UPDATE_CLUSTER)
  int updateCluster(@BindBean Cluster newCluster);

  @SqlUpdate(SQL_DELETE_CLUSTER)
  int deleteCluster(@Bind("name") String clusterName);
  ...
  @Mapper(RepairRunMapper.class)
  Collection<RepairRun> getRepairRunsForCluster(@Bind("clusterName") String clusterName);

  @SqlQuery(SQL_GET_REPAIR_RUNS_WITH_STATE)
  @Mapper(RepairRunMapper.class)
  Collection<RepairRun> getRepairRunsWithState(@Bind("state") RepairRun.RunState state);

  @SqlQuery(SQL_GET_REPAIR_RUNS_FOR_UNIT)
  @Mapper(RepairRunMapper.class)
  Collection<RepairRun> getRepairRunsForUnit(@Bind("unitId") long unitId);

  @SqlUpdate(SQL_INSERT_REPAIR_RUN)
  @GetGeneratedKeys
  long insertRepairRun(@BindBean RepairRun newRepairRun);

  @SqlUpdate(SQL_UPDATE_REPAIR_RUN)
  int updateRepairRun(@BindBean RepairRun newRepairRun);
  ...
  RepairUnit getRepairUnit(@Bind("id") long repairUnitId);

  @SqlQuery(SQL_GET_REPAIR_UNIT_BY_CLUSTER_AND_TABLES)
  @Mapper(RepairUnitMapper.class)
  RepairUnit getRepairUnitByClusterAndTables(@Bind("clusterName") String clusterName,
  ...
  @GetGeneratedKeys
  long insertRepairUnit(@BindBean RepairUnit newRepairUnit);

  @SqlUpdate(SQL_DELETE_REPAIR_UNIT)
  int deleteRepairUnit(@Bind("id") long repairUnitId);

  @SqlBatch(SQL_INSERT_REPAIR_SEGMENT)
  @BatchChunkSize(500)
  void insertRepairSegments(@BindBean Iterator<RepairSegment> newRepairSegments);

  @SqlUpdate(SQL_UPDATE_REPAIR_SEGMENT)
  int updateRepairSegment(@BindBean RepairSegment newRepairSegment);

  @SqlQuery(SQL_GET_REPAIR_SEGMENT)
  @Mapper(RepairSegmentMapper.class)
  RepairSegment getRepairSegment(@Bind("id") long repairSegmentId);
  ...
  int deleteRepairSegmentsForRun(@Bind("runId") long repairRunId);
  ...
  @GetGeneratedKeys
  long insertRepairSchedule(@BindBean RepairSchedule newRepairSchedule);

  @SqlUpdate(SQL_UPDATE_REPAIR_SCHEDULE)
  int updateRepairSchedule(@BindBean RepairSchedule newRepairSchedule);
  ...
  int deleteRepairSchedule(@Bind("id") long repairScheduleId);
